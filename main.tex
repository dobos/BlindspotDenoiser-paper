...
Wavelet BayesShrink & classical filter & $26.6$ & $24.2$ [$13.4$, $48.3$] \\
PCA, $k\!=\!16$ (noisy-trained) & linear, noisy-only & $54.0$ & $51.0$ [$26.4$, $91.0$] \\
PCA, $k\!=\!16$ (clean-template) & linear, clean reference & $55.7$ & $52.9$ [$27.1$, $93.0$] \\
\textbf{Blindspot output} & \textbf{self-supervised (ours)} & $\mathbf{122}$ & $\mathbf{109}$ [$\mathbf{45}$, $\mathbf{246}$] \\
\hline
\end{tabular}
\end{table}

The clean-template PCA reference isolates the limiting factor. It is built from noiseless training spectra, so it carries the clean targets the blindspot model never sees, yet it reaches only $55.7$, within a few percent of the noisy-only PCA at $54.0$. The ceiling is set by the linear estimator class, not by access to supervision. Two structural limits hold the linear baselines there. First, each spectrum carries a radial velocity spanning $\pm300$~km/s (Table~\ref{tab:data_summary}), so its absorption lines sit at spectrum-dependent pixel positions, displaced by several resolution elements across the sample. A principal-component reconstruction projects onto a fixed set of vectors on a fixed wavelength grid. The velocity-shifted copies of a line do not lie in any low-dimensional span of that grid, since representing an arbitrary shift requires all Fourier modes. The rank needed therefore grows with the velocity range, not with the number of stellar parameters, and a rank-$16$ basis cannot register lines it cannot translate; clean targets do not change this, and shifting to a rest frame is not a free repair either (Section~\ref{sec:discussion}). Second, even at fixed velocity the line cores are high-frequency, low-variance features. Recovering them demands higher rank, but higher rank readmits the noise directions that the low rank suppresses, so the validation-selected rank saturates at the smallest value searched ($k=16$ over $\{16,\dots,512\}$). The blindspot estimator is subject to neither limit. It predicts each bin as a nonlinear function of a local neighborhood with the central bin excluded, so it learns the map from line shape to central flux rather than a basis indexed by absolute wavelength, and a global velocity shift leaves that map unchanged. Its accuracy is then set by the physical correlation length of the signal relative to the per-bin noise (Section~\ref{sec:theory}), not by a subspace dimension. The central-pixel exclusion behind this is verified numerically, and a supervised clean-target U-Net bounds the headroom from paired clean data, in Appendix~\ref{app:architecture_integrity}.
...
\centering
\small
\caption{Ablation by training signal on the test split. Mean S/N is the linear per-spectrum reconstruction statistic in Equation~\ref{eq:snr_def} averaged over the $10{,}000$ test spectra under the same evaluation protocol as Table~\ref{tab:main_baselines}. The Median S/N [$5\%, 95\%$] column reports the per-spectrum distribution over the same test spectra. The supervised U-Net row is a clean-target reference because it requires paired clean targets unavailable to a deployed self-supervised system.}
\label{tab:training_signal_ablation}
\begin{tabular}{llccc}
\hline
Training signal & Loss target & $\bar{\sigma}$ input & Mean S/N & Median S/N [$5\%, 95\%$] \\
\hline
Central-visible (no blindspot) & noisy $y$ & none & $7.6$ & $6.8$ [$3.0$, $14.6$] \\
Blindspot, no $\bar{\sigma}$ input & noisy $y$ & none & $118$ & $104$ [$43$, $239$] \\
\textbf{Blindspot $+$ scalar $\bar{\sigma}$} & noisy $y$ & scalar $\bar{\sigma}$ & $\mathbf{122}$ & $\mathbf{109}$ [$\mathbf{45}$, $\mathbf{246}$] \\
Supervised U-Net (clean-target reference) & clean $x$ & none & $154$ & $136$ [$55$, $319$] \\
\hline
\end{tabular}
\end{table}
...
\hline
Noisy input & $7.6$ & $6.8$ [$3.1$,$14.6$] & $3.09/0.845$ & $0.282$ \\
Savitzky--Golay & $20.8$ & $19.1$ [$9.7$,$38.2$] & $1.89/0.588$ & $0.121$ \\
Gaussian smoothing & $25.8$ & $23.1$ [$13.2$,$47.5$] & $1.49/0.662$ & $0.0842$ \\
Median filter & $15.7$ & $14.3$ [$6.7$,$29.1$] & $2.21/0.645$ & $0.168$ \\
Wavelet BayesShrink & $26.6$ & $24.2$ [$13.4$,$48.3$] & $1.50/0.695$ & $0.0804$ \\
Noisy-PCA, $k=16$ & $54.0$ & $51.0$ [$26.4$,$91.0$] & $1.09/0.338$ & $0.0184$ \\
Clean-template PCA reference, $k=16$ & $55.7$ & $52.9$ [$27.1$,$93.0$] & $1.02/0.318$ & $0.0172$ \\
\hline
\end{tabular}
\end{table}
