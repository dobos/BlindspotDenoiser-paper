...
\centering
\small
\caption{Ablation by training signal on the test split. Mean S/N is the linear per-spectrum reconstruction statistic in Equation~\ref{eq:snr_def} averaged over $1000$ test spectra with $10$ noise realizations each, under the same evaluation protocol as Table~\ref{tab:main_baselines}. The Median S/N [$5\%, 95\%$] column reports the per-spectrum distribution over the same test spectra. The supervised U-Net row is a clean-target reference because it requires paired clean targets unavailable to a deployed self-supervised system.}
