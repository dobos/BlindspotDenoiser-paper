...
The three terms encode noise from object photons, photon noise from the structured sky background, and detector read and dark noise as transfered by the telescope and the detector. We note, that $\sigma_{\mathrm{obj},\lambda}^2$ is a deterministic function of the clean flux $x$, hence the full, pixel-wise $\sigma_{\lambda}$ profile is used only to draw $\epsilon_{\lambda}$ in Equation~\ref{eq:noise_model_scalar}. The variance vector is never fed into the denoiser directly, because it would leak information about the clean flux into the network. The network receives only the scalar noise summary $\bar{\sigma}$, as defined in Equation~\ref{eq:blindspot_prediction}. The clean reference $x$ is completely withheld during training and used only for evaluation.

\subsection{Problem Formulation and Blindspot Objective}
\label{sec:problem_formulation}

Let $x\in\mathbb{R}^{N}$ be a clean spectrum and $y\in\mathbb{R}^{N}$ its noisy observation:
\begin{align}
    y_{\lambda} &= x_{\lambda} + \epsilon_{\lambda} \\[1em]
    \epsilon_{\lambda} &\sim \mathcal{N}(0,\sigma_{\lambda}^{2}),
    \label{eq:noise_model_scalar}
\end{align}
where $\lambda = 1,2,\dots, N$ indexes the wavelength bins.

The task is to learn the $\theta$ parameters of a denoiser $f_\theta$ from noisy spectra only: during training, $y$ is used but $x$ is not. A blindspot predictor estimates each wavelength bin from its surrounding context with the center bin excluded:
\begin{align}
    \hat{x}_{\lambda} 
        &= f_{\theta}(y_{-\lambda},\bar{\sigma}^2)
        \label{eq:blindspot_prediction} \\
    \bar{\sigma}^2
        &= \frac{1}{N}\sum_{\lambda=1}^{N}\sigma_{\lambda}^{2}.
\end{align}
Here $y_{-\lambda}$ denotes the receptive-field input after excluding $y_{\lambda}$, $\hat{x}_{\lambda}$ is the point estimate returned by the network, and $\theta$ denotes all trainable weights and biases of the denoiser. The scalar $\bar{\sigma}^2$ represents the overall noise level of the spectrum and $\bar{\sigma}$ has the same flux unit as $y$. We use the sum of the per-bin variances rather than the full vector $\{\sigma_{\lambda}\}_{\lambda=1}^{N}$ because the full vector contains wavelength-specific information from the noiseless simulated spectrum that could leak into the prediction. In this experiment, the pixel-wise variance is only used to generate the simulated noise, but the denoiser receives only the scalar summary $\bar{\sigma}$.
...
\begin{equation}
    \theta^\ast = \operatorname*{arg\,min}_{\theta}\;\mathbb{E}_{y}\!\left[\mathcal{L}(\theta;y)\right],
    \label{eq:theta_star}
\end{equation}
where the expectation value is taken over the ensemble of noisy spectra. The denoised spectrum is obtained by evaluating Equation~\ref{eq:blindspot_prediction} at $\theta^\ast$.

Note, that neither clean fluxes nor per-pixel inverse-variance weights enter the training loss. The pixelwise variance vector is compressed into the scalar $\bar{\sigma}$ before it is supplied to the network. Under the symmetric zero-mean per-pixel noise model of Equation~\ref{eq:noise_model_scalar}, and for a Huber breakpoint $\delta$ chosen to be large relative to the typical residuals, the \ld{population minimizer is approximately}
\begin{equation}
    f^\ast(y_{-\lambda},\bar{\sigma}^2)
    \;\approx\;
    \mathbb{E}\!\left[y_{\lambda}\mid y_{-\lambda},\bar{\sigma}^2\right]
    \;=\;
    \mathbb{E}\!\left[x_{\lambda}\mid y_{-\lambda},\bar{\sigma}^2\right].
    \label{eq:population_target}
\end{equation}
The equality follows because the noise has zero conditional mean. Asymmetric or heavy-tailed noise, or a $\delta$ comparable to or smaller than the residual scale, would shift the optimum toward an outlier-attenuated conditional location rather than the conditional mean.
...
as long as the forward pass ensures one invariant: the feature used to predict the flux in bin $\lambda$ is never computed from $y_{\lambda}$. When feature extraction is done with a convolutional network, this property is often referred to as \textit{causality}. To compute some internal features that will be used downstream to predict $\hat{x}_{\lambda}$, the one-dimensional blindspot network uses two identical, but mirrored causal branches. The left branch calculates the feature vector $\m{u}_{\lambda}^{L} = U_{\theta}(\m{z}^{L})$, where $\m{z}^{L}$ is the input spectrum in the normal order of wavelength bins for all $\lambda_i < \lambda$, so the extracted features at $\lambda$ depend only on $y_{<\lambda}$. The right branch processes the wavelength-reversed spectrum for all $\lambda_i > \lambda$ and calculates $\m{u}_{\lambda}^{R} = U_{\theta}(\m{z}^{R})$, so the right-side features at $\lambda$ depend only on $y_{>\lambda}$. The prediction head, that calculates $\hat{x}_{\lambda}$, receives only the $\m{u}_{\lambda}^{L}$ and $\m{u}_{\lambda}^{R}$ feature vector, therefore, no computational path carries $y_{\lambda}$ into $\hat{x}_{\lambda}$.

\begin{figure}
    \begin{center}
    \includegraphics{figs/network.pdf}
    \label{fig:network}
    \caption{Illustration of the blindspot network architecture for the prediction of a single spectrum pixel $\hat{x}_{\lambda}$. The two UNet blocks of $U_{\theta}$ are identical with the same weights but their receptive fields are only $y_{<\lambda}$ and $y_{>\lambda}$, respectively and $y_{\lambda}$ is excluded from both. The left block processes the input spectrum in increasing wavelength order, while the input of the right block is reversed in wavelength order. The two feature vectors $u_{\lambda}^{L}$ and $u_{\lambda}^{R}$ are concatenated and passed to the pointwise head $g_{\theta}$ to predict $\hat{x}_{\lambda}$.}
    \end{center}
\end{figure}

The input to the blindspot network is a 1-dimensional array $y\in\mathbb{R}^{L}$ representing the continuum-normalized flux, ordered by wavelength and $\lambda$, the index of the wavelength bin of interest. The output of the network is $\hat{x}_\lambda$, where each $\hat{x}_{\lambda}$ depends on $y_{<\lambda}$ and $y_{>\lambda}$ but not on $y_{\lambda}$.
...
\begin{enumerate}
    ...
    \item Apply the same $U_{\theta}$ to each branch shifted to the wavelength of interest:
...
\paragraph{Identifiability follows from zero-mean noise.} Let $\Omega_{\lambda} = (y_{-\lambda},\bar{\sigma})$ denote the blindspot context. Identifiability follows from
\begin{align}
    y_{\lambda} = x_{\lambda} + \epsilon_{\lambda} \\[1em]
...
The blindspot construction is useful only when the spectral pixels surrounding the blind spot determine the clean flux at the hidden wavelength better than the noisy observation itself. We measure the recoverability by the ratio between two variances: let $\tau_\lambda^{2}$ denote the conditional clean-flux variance left after observing the blindspot context, and let $\sigma_\lambda^{2}$ denote the per-pixel noise variance. We denote this ratio by $\gamma_\lambda$ and express it in terms of the conditional variance of the clean flux and the variance of the noise as
\begin{align}
    \gamma_\lambda &=\; \frac{\tau_\lambda^{2}}{\sigma_\lambda^{2}} \\[0.5em]
    \sigma_\lambda^{2} &= \mathrm{Var}(\epsilon_{\lambda}) \\[1em]
    \tau_\lambda^{2} &= \mathrm{Var}(x_{\lambda}\mid\Omega_{\lambda}).
    \label{eq:gamma}
\end{align}
...
The observation simulator outputs the $\sigma_{\lambda}^{2}$ per-pixel variances, see Eq.~\ref{eq:noise_components}, while the network receives the spectrum-level scalar $\bar{\sigma}^{2}=L^{-1}\sum_{\lambda}\sigma_{\lambda}^{2}$. The scalar $\gamma$ in Equation~\ref{eq:gamma} therefore describes spectrum-averaged recoverability under the same averaging used for conditioning. Wavelength windows whose local variance profile differs strongly from $\bar{\sigma}^{2}$, such as the \mbox{O$_2$} A-band, require direct window diagnostics. We will evaluate those windows in Section~\ref{sec:results_physical} instead of treating the scalar $\gamma$ as a local guarantee.

\paragraph{The target support has a physically set correlation scale}
The reason to expect small $\gamma$ is not generic smoothness, but the ratio between the spectral correlation length and the \ld{per-bin noise scale ??}. The spectral correlation length is determined jointly by stellar physics via intrinsic line width and by the instrument via the line-spread function.
...
Spectra with different surface gravity distributions, or a different line-spread function should be checked separately rather than assumed to share the same small-$\gamma$ range. We therefore use the small-$\gamma$ argument as a range statement for this simulated data set, not as an unconditional guarantee beyond it.

\paragraph{From pixel recoverability to non-linear downstream tasks}
The recoverability ratio in Equation~\ref{eq:gamma} concerns pixel-level reconstruction. Downstream tasks, such as surface gravity measurement, see Section~\ref{sec:results_downstream_logg}, are non-linear functionals of the full spectrum and are not directly bounded by $\gamma$. Hence, we make the weaker claim that a blindspot denoiser can recover the line-shape statistics well enough so that a downstream non-linear regressor, trained solely on clean spectra, can make use of the denoised spectra without retraining.
...
This is consistent with the calibration transfer reported below, where a LightGBM gravity regressor trained on clean spectra and evaluated without refitting is used to predict $\log g$ from the noisy and the denoised spectra. The blindspot denoiser raised the coefficient of determination from $R^{2}\approx0.01$ to $R^{2}\approx0.46$ and removed most of the noisy-input systematic bias. As a contrast, results from using simple pixel-domain smoothers remain close to the noisy-input value.
Calculating a formal information-channel bound for this propagation is left for future work. The present paper treats the calibration result as empirical confirmation of the line-preservation diagnostics in Section~\ref{sec:results_physical}.
