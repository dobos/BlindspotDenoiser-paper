...
\begin{itemize}
    ...
    \item Line-sensitive and high-noise-window diagnostics evaluated alongside aggregate reconstruction error: Ca\,\textsc{ii} triplet equivalent-width (EW) error, continuum-relative polarity-flip rate, and an \mbox{O$_2$} A-band stress test in a high-noise atmospheric window.
\end{itemize}
...
% ROLE: morphology evidence that line preservation is not only an aggregate metric effect.
\begin{figure}[htbp]
\centering
...
The fixed map extracts gravity-discriminating information from clean inputs on this BOSZ support, and the blindspot output moves the same estimator away from the noisy-input scale across scatter, absolute error, bias, and $R^{2}$. The improvement remains short of the clean-reference row, so the result is a substantial transfer gain rather than full clean-domain recovery. Figure~\ref{fig:downstream_clean_domain_transfer} shows the $\log g$ residual distributions together with a four-label context panel that is an auxiliary consistency check rather than a full stellar-label recovery claim; the calibrator train-size sweep is reported in Appendix~\ref{app:downstream_logg}.

% ROLE: main-text downstream residual visualization and auxiliary four-label context; not a full all-label recovery claim.
\begin{figure}[htbp]
\centering
...
Reconstruction signal-to-noise rises with estimator capacity, but the downstream $\log g$ estimator separates methods that preserve label information from those that only raise pixel S/N: the principal-component references reach the highest reconstruction S/N among the non-neural methods yet reduce the fixed calibrator's downstream accuracy relative to the noisy-input row, whereas the blindspot moves the calibrator toward the clean-input scale.
...
\hline
Method & Mean S/N & Median S/N [$5$,$95$] & Ca\,\textsc{ii} EW (mean) & All-pixel polarity & $\log g\ \sigma_{\rm rob}$ & $\log g\ R^2$ \\
\hline
