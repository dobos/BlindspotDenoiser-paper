...
We evaluate one frozen blindspot checkpoint in the simulated BOSZ/PFS-like low-S/N benchmark. The model is trained only on noisy spectra; clean spectra enter only after training, for reconstruction metrics and downstream tests. The Results section asks three questions: whether the reconstruction recovers the spectrum, whether clean-trained stellar-label estimators can read the denoised spectrum, and whether the absorption-line features are preserved rather than smoothed away. All numbers in Sections~\ref{sec:results_denoising} and~\ref{sec:results_physical} are measured on the canonical held-out reconstruction split of $1000$ spectra with $10$ independent noise realizations per spectrum; the downstream calibrator in Section~\ref{sec:results_downstream_logg} uses a separate $5{,}000$-spectrum held-out test partition of the same scope-restricted BOSZ library, disjoint from the calibrator-training set. Clean spectra are used only for evaluation.

\subsection{Reconstruction performance on the held-out spectra}
\label{sec:results_denoising}

Figure~\ref{fig:representative_spectrum} shows the central result on a single faint spectrum: the noisy input obscures the Ca\,\textsc{ii} triplet, while the blindspot output follows the clean line cores without using paired clean targets at training time. Aggregated over the held-out reconstruction set, Table~\ref{tab:main_baselines} shows the same effect quantitatively: mean reconstruction S/N rises from $7.8$ on the noisy input to $124$ on the blindspot output, a factor of about $16$.
...
The mechanism is the combination of an unavailable central pixel and an informative spectral context. During training, the target wavelength contributes only through the noisy loss value; the prediction itself is computed from $y_{-\lambda}$ and a scalar noise condition. The zero-mean independent noise model makes the noisy target an unbiased training signal in aggregate, while the masked spectral context retains enough information about line cores and continuum structure to predict the withheld bin. The scalar $\bar{\sigma}$ can tell the network the overall noise scale of a spectrum, but it cannot encode where Ca\,\textsc{ii} or other wavelength-local features should appear.
...
The comparison is stronger than ``better than the noisy input.'' The blindspot row clears the validation-tuned pixel-domain filter, the noisy-only PCA projection, and the non-deployable clean-template PCA reference, so the gain is not explained by importing clean targets, by tuning a conventional smoother, or by a fixed-rank linear reconstruction of the training support. The input--output S/N density in Figure~\ref{fig:appendix_snr_panel} also stays above the identity line across the input-S/N decade, so the headline gain is not concentrated in a small subset of easy spectra. A perturbation audit verifies the central-pixel exclusion property numerically, and a supervised clean-target U-Net reference bounds the improvement available with paired clean training data; both are detailed in Appendix~\ref{app:architecture_integrity}.

\subsection{Downstream clean-trained calibrator transfer}
\label{sec:results_downstream_logg}

The reconstruction metric is not the endpoint for Galactic Archaeology. A denoiser is useful only if spectra reconstructed from noisy observations remain readable to downstream estimators that were trained on clean spectra. We therefore train full-spectrum stellar-label calibrators on clean spectra, freeze them, and apply them unchanged to noisy, smoothed, and blindspot-denoised held-out spectra.

The calibrator is a gradient-boosted regression-tree model (LightGBM, default hyperparameters) fit on the full continuum-normalized flux vector of $30{,}000$ clean training spectra. Calibrator training and evaluation use a labeled BOSZ pool of $50{,}000$ spectra drawn from the same scope-restricted library as the reconstruction split, partitioned into a $30{,}000$-spectrum calibrator-training set and a disjoint $5{,}000$-spectrum held-out test set. The trained model is then frozen and applied unchanged to five representations of the same held-out spectra: clean input, blindspot output $\hat{x}$, noisy input $y$, the validation-tuned box-car smoother, and the validation-tuned Savitzky--Golay smoother. The clean-input row is an upper reference for what the fixed map can extract under the simulator label distribution; it is not a deployable comparison.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig4_downstream_clean_domain_transfer.pdf}
\caption{Frozen clean-trained downstream calibration after denoising. Full-spectrum LightGBM calibrators are trained only on clean spectra and then evaluated, without refitting, on the same held-out spectra represented as clean inputs, blindspot outputs, box-car smooths, Savitzky--Golay smooths, or noisy inputs. \textbf{Left:} centered robust-scatter ratio relative to the noisy input at $n_{\rm train}=30{,}000$ for four BOSZ labels; small points are five random splits, large markers are split means, and bars are split standard deviations. \textbf{Right:} residual distributions for the primary $\log g$ audit on the $5{,}000$-spectrum held-out partition in Table~\ref{tab:downstream_logg}; side labels report the mean bias $\mu$ and centered robust scatter $\sigma$ in dex. The paired-bootstrap blindspot/noisy ratios for this $\log g$ split are $0.64$ $[0.61,\,0.66]$ for $\sigma_{\rm rob}$ and $0.70$ $[0.68,\,0.72]$ for MAE.}
...
\hline
Clean input (upper reference)         & $0.222$ & $0.205$ & $+0.001$ & $0.940$ \\
...
Figure~\ref{fig:downstream_clean_domain_transfer} shows that the blindspot output transfers better than noisy or conventionally smoothed spectra across all four labels. At the maximum training size, the blindspot/noisy robust-scatter ratios are $0.16$ for $T_{\rm eff}$, $0.63$ for $\log g$, $0.31$ for $[\mathrm{M/H}]$, and $0.14$ for RV, with the same ordering across the five split seeds. The classical smoothers can reduce scatter for some labels, but they remain above the blindspot point in every label and stay close to the noisy-input scale for the surface-gravity and metallicity audits.

The $\log g$ row is the primary downstream audit because surface gravity is the label most directly tied to dwarf--giant selection. The clean-input row reaches $R^{2}=0.940$, so the task itself is not degenerate: the fixed mapping encodes gravity-discriminating spectral information in this BOSZ support. Against that reference, the blindspot output recovers the information that the noisy input loses --- the centered robust scatter falls from $1.262$ on the noisy input to $0.801$~dex, MAE falls from $0.958$ to $0.670$~dex, and the noisy-input bias is largely removed. The two validation-tuned smoothers do not transfer for this primary audit: their residual statistics stay close to the noisy-input scale. The improvement therefore depends on the line-preserving reconstructions characterized in Section~\ref{sec:results_physical}, not on generic noise reduction by a conventional smoother.
...
The downstream gain reported in Section~\ref{sec:results_downstream_logg} would be uninformative if the denoiser achieved high aggregate S/N by filling absorption features toward the continuum. We therefore test line-sensitive diagnostics directly on the same held-out reconstruction split used in Section~\ref{sec:results_denoising}: relative equivalent-width error on each Ca\,\textsc{ii} triplet line, continuum-relative polarity flips, and the sky-dominated \mbox{O$_2$} A-band atmospheric window.
...
$\quad 8662$~\AA{} & $1.025$ & $\mathbf{0.143}$ \\
\hline
...
The three Ca\,\textsc{ii} rows show that the recovery is not a single-line accident. The strongest-line measurement at $8542$~\AA{} preserves the line core ($0.338$ on the noisy input becomes $0.092$ on the blindspot output); the hardest-line measurement at $8662$~\AA{} shows the largest absolute recovery ($1.025$ to $0.143$); and the $8498$~\AA{} row confirms the same direction at intermediate strength. The polarity row is the sharper check against smoothing: continuum-relative sign mistakes become rare after denoising rather than merely smaller in amplitude. The \mbox{O$_2$} A-band rows add a stress test in a narrow, high-noise atmospheric window; they show that this window is not treated as disposable background under the independent-noise simulator. Figure~\ref{fig:caII_zoom} supplies the morphology behind the table: the blindspot output follows the depth and width of the line cores rather than flattening them toward the continuum.

The mechanism that supports the downstream gain is therefore line preservation, not generic smoothing. Validation-tuned smoothers and PCA references underperform in Table~\ref{tab:main_baselines}; central-pixel leakage is ruled out structurally by the blindspot construction and numerically by the perturbation audit in Appendix~\ref{app:architecture_integrity}; the context-predictability premise that justifies blindspot training in this support is verified with model-free predictors in Appendix~\ref{app:controls_gamma}. The training-signal ablation against a supervised clean-target U-Net reference is reported in the same appendix. All claims in this section are made for continuum-normalized, medium-resolution, late-type BOSZ/PFS-like spectra at magnitude $20.5$--$22.5$ with independent zero-mean per-pixel noise, which is the simulated benchmark certified here.
...
\paragraph{Downstream information preservation.} The frozen clean-trained $\log g$ calibrator is the strongest evidence that the denoised spectra preserve scientifically usable information. It uses the full flux vector rather than the six equivalent-width diagnostics, and it is never retrained on noisy or denoised spectra. The improvement is therefore a transfer result: the representation learned from clean spectra remains usable after blindspot denoising. The failure of the validation-tuned smoothers on the same frozen estimator is an important differential control, because it shows that the downstream gain is not a generic consequence of making spectra smoother. Improvements in $\log g$ residual statistics are not equivalent to a thresholded dwarf--giant classification metric; a confusion matrix on labeled samples is left for follow-up work.

\section{Conclusion}
\label{sec:conclusion}
This paper establishes, for a continuum-normalized late-type BOSZ/PFS-like simulator with independent zero-mean per-pixel noise, that a one-dimensional self-supervised blindspot denoiser trained only on noisy spectra raises low-S/N reconstruction quality, preserves the absorption-line structure that downstream stellar analysis depends on, and remains readable to a frozen clean-trained surface-gravity calibrator. Each of these is reported as a stand-alone diagnostic rather than a final classification metric: reconstruction is compared against validation-tuned classical baselines and PCA references, line preservation against the diagnostics that would expose continuum filling, and downstream transfer through an unmodified estimator applied in turn to denoised, smoothed, and noisy spectra.

The result is therefore a transfer claim, not a deployment claim. It shows that noisy-only training is sufficient for the line-shape and continuum information that a frozen clean-trained estimator needs on this benchmark, and it leaves open whether the same construction transfers under correlated sky-subtraction residuals, extraction covariance, varying instrumental line-spread function, or stellar regimes outside the late-type, faint-magnitude scope certified here. Whether residual gains in surface-gravity scatter translate into a thresholded dwarf--giant classification metric on labeled samples, and whether the line-preservation diagnostics extend to abundance and radial-velocity measurements, are the natural next tests under the same frozen-evaluation protocol.
