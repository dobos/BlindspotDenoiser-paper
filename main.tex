...
\begin{itemize}
    ...
    \item The main downstream result: a full-spectrum surface-gravity ($\log g$) estimator trained on clean spectra and evaluated without refitting transfers substantially better to blindspot-denoised spectra than to noisy or validation-tuned smoothed inputs, recovering much of the gravity-discriminating information used for dwarf--giant separation in faint Galactic Archaeology samples, with the coefficient of determination rising from $0.01$ on noisy input to $0.46$ against a $0.94$ clean-input ceiling.
...
\caption{Canonical simulated noise configuration used in the main text. Input S/N is the per-spectrum linear ratio in Equation~\ref{eq:snr_def} evaluated with $X{=}y$. The input distribution over test spectra has mean $7.8$ and median $7.8$ with $5$--$95\%$ range $[3.0,14.5]$; the corresponding blindspot-output distribution and its $95\%$ confidence interval are reported in Table~\ref{tab:main_baselines}.}
\begin{tabular}{ll}
\hline
...
Test-set noise realizations & $10$ per test spectrum \\
...
On the reconstruction test split, mean per-spectrum reconstruction S/N rises from $7.8$ on the noisy input to $124$ on the blindspot output: a factor of about $16$, on a metric (Equation~\ref{eq:snr_def}) that compares each denoised spectrum against the withheld clean reference (Table~\ref{tab:main_baselines}). Figure~\ref{fig:representative_spectrum} shows one faint spectrum where the input drowns the Ca\,\textsc{ii} triplet and the output follows the line cores. The canonical reconstruction test split contains $1000$ spectra with $10$ independent noise realizations each and no clean reference at training time (Appendix~\ref{app:contract}).
...
\centering
\small
\caption{Clean-trained $\log g$ estimator on five reconstruction inputs, evaluated without refitting on $5{,}000$ test spectra. $\sigma_{\rm rob}$ is the centered Gaussian-equivalent dispersion of the residual $\widehat{\log g}-\log g_{\rm true}$; MAE and bias are the mean absolute and mean signed residuals; $R^{2}$ is the coefficient of determination. The clean-input row is an upper reference under the fixed mapping. Paired-bootstrap $95\%$ confidence intervals over $1000$ resamples on the blindspot-vs-noisy ratios are $\sigma_{\rm rob}$-ratio $0.64$ $[0.61,\,0.66]$ and MAE-ratio $0.70$ $[0.68,\,0.72]$.}
\label{tab:downstream_logg}
\begin{tabular}{lcccc}
\hline
Input spectrum & $\sigma_{\rm rob}$ [dex] & MAE [dex] & Bias [dex] & $R^{2}$ \\
\hline
Clean input (upper reference)         & $0.22$ & $0.21$ & $+0.001$ & $0.94$ \\
\textbf{Blindspot output (ours)}      & $\mathbf{0.80}$ & $\mathbf{0.67}$ & $\mathbf{+0.020}$ & $\mathbf{0.46}$ \\
Box-car smoother (val.\ tuned)        & $1.09$ & $0.87$ & $-0.086$ & $0.14$ \\
Savitzky--Golay smoother (val.\ tuned)& $1.09$ & $0.87$ & $-0.089$ & $0.13$ \\
Noisy input                           & $1.26$ & $0.96$ & $-0.170$ & $0.01$ \\
\hline
\end{tabular}
\end{table}

The clean-reference $R^{2}$ in Table~\ref{tab:downstream_logg} establishes that the fixed map extracts gravity-discriminating information on this BOSZ support. The blindspot row inherits that information channel: scatter, mean absolute error, and systematic bias all move from the noisy-input scale toward the clean-reference scale, while validation-tuned smoothers in the same configuration stay near the noisy-input scale across the metrics. The bias row makes the contrast sharpest: blindspot output nearly removes the noisy systematic offset (\mbox{$+0.02$} versus $-0.17$~dex), while the smoothers remain offset at about $-0.09$~dex. The reduction therefore tracks the line-preserving reconstructions of Section~\ref{sec:results_physical}, not generic noise removal.

Across the four BOSZ labels in Figure~\ref{fig:downstream_clean_domain_transfer}, the blindspot point sits below both the noisy point and the two classical smoother points at every label, and the same ordering holds across the five split seeds. The labels separate into an easier temperature/radial-velocity group and a harder gravity/metallicity group, matching the line-shape and line-strength sensitivity of these targets. The validation-tuned smoothers reduce scatter for some labels but stay close to the noisy-input scale for surface gravity and metallicity, so they do not substitute for the blindspot reconstruction.
...
\caption{Line-sensitive reconstruction diagnostics on the test split. Equivalent-width entries use the relative error in Equation~\ref{eq:ew_relative_error}; polarity flips use Equation~\ref{eq:polarity_flip}. Per-line bootstrap confidence intervals and additional rows are reported in Appendix~\ref{app:line_addenda}, Table~\ref{tab:main_results_full}.}
...
\noindent The appendices are organized as follows. Appendix~\ref{app:contract} specifies the evaluation protocol and reproducibility contract. Appendix~\ref{app:architecture_integrity} documents the canonical blindspot model and the numerical leakage audit. Appendix~\ref{app:controls_gamma} reports the validation-tuned baseline grids and the context-variance ($\gamma$) estimators. Appendix~\ref{app:line_addenda} gives the distributional checks of the reconstruction and line-preservation results. Appendix~\ref{app:downstream_logg} reports the downstream gravity-calibration train-size sweep. Appendix~\ref{app:noise_stability} reports the repeated-noise stability of the reconstruction.
...
\small
\setlength{\tabcolsep}{4pt}
\caption{Artifacts defining the evaluation protocol. Each entry resolves to an asset inside the archived release; the same release pins the selected checkpoint, the test split, and the metric implementation used for every quantitative result in this paper.}
...
The reconstruction training, validation, and test sets contain $200{,}000$, $1{,}000$, and $1{,}000$ spectra respectively, drawn without replacement from the same scope-restricted BOSZ library so that the marginal distributions over $T_{\rm eff}$, $\log g$, $[\mathrm{M}/\mathrm{H}]$, and magnitude match across splits. All hyperparameter selection --- the training-time validation rule for the blindspot model, the classical-baseline grids in Appendix~\ref{app:controls_gamma}, and the model-free $\gamma$ predictors --- uses the validation split alone, with a single fixed validation noise seed. After selection, the hyperparameters are held fixed, and the reconstruction and line-preservation results are computed on the reconstruction test split with $10$ independent noise realizations per spectrum drawn from a deterministic seed sequence. Per-spectrum metrics are first averaged across spectra at each realization and then across realizations; reported $\pm$ values are $95\%$ confidence intervals over the realizations.
...
\hline
Cross-spectrum permutation & $3$ & $64$ & $1.79\times10^{-7}$ \\
Per-spectrum mean replacement & $3$ & $64$ & $2.09\times10^{-7}$ \\
\hline
\end{tabular}
\end{table}
...
\centering
\small
\caption{Ablation by training signal on the test split. Mean S/N is the linear per-spectrum reconstruction statistic in Equation~\ref{eq:snr_def} averaged over the $10000$ test spectra under the same evaluation protocol as Table~\ref{tab:main_baselines}. The Median S/N [$5\%, 95\%$] column reports the per-spectrum distribution over the same test spectra. The supervised U-Net row is a clean-target reference because it requires paired clean targets unavailable to a deployed self-supervised system.}
...
This appendix provides the train-size stability check referenced in Section~\ref{sec:results_downstream_logg}. The same clean-trained gradient-boosted regressor is refit at four training-size operating points, holding the test split and evaluation protocol fixed; only the calibrator's training-size hyperparameter changes between rows.
