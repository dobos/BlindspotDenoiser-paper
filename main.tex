...
\begin{itemize}
    ...
    \item We compared the performance of the blindspot denoiser against validation-tuned classical denoising filters, a denoiser based on principal-component analysis (PCA) using noisy input spectra and a non-deployable clean-template PCA reference, using aggregate reconstruction S/N as measure of performance.
    \item We performed line-preservation diagnostics by measuring the reconstruction error of the Ca\,\textsc{ii} triplet equivalent-width (EW), the continuum-relative polarity-flip rate, and an \mbox{O$_2$} A-band test in a high-noise atmospheric window.
\end{itemize}
...
\begin{table*}
    ...
    \caption{Paramaters of the ensemble of simulated spectra used in the main text. Input S/N is the per-spectrum linear ratio in Equation~\ref{eq:snr_def} evaluated with $X{=}y$. The distribution of S/N over the test spectra has a mean of $7.6$ and median of $6.8$ with a $5$--$95\%$ range of $[3.1,14.6]$. The corresponding blindspot-output distribution of S/N and its $95\%$ confidence interval are reported in Table~\ref{tab:main_baselines}.}
    \begin{tabular}{ll}
    \hline
...
\paragraph{Signal-to-noise ratio} Since in case of simulated spectra, we know the clean reference, as well as the noise variance as a function of wavelength, but we cannot calculate the latter for the denoised output, we use a definition of S/N that can be calculated for both. Writing $z\in\{y,\hat{x}\}$ for either the noisy input or the denoiser output evaluated against the clean reference $x$, signal-to-noise is defined as
\ld{???
\begin{equation}
    \mathrm{S/N}(z)
    \;=\;
    \frac{\lVert x\rVert_{2}}{\lVert x-z\rVert_{2}}
    % \qquad
...
Whenever reconstruction S/N is reported from now on, it is the \ld{per-spectrum norm ratio} in Equation~\ref{eq:snr_def} evaluated against the clean test reference. \ld{We do not report a variance-normalized alternative; aggregations over wavelength enter only through the $\ell_{2}$ norms in Equation~\ref{eq:snr_def}}, and aggregations over spectra are performed only after this per-spectrum quantity has been computed.
...
On the reconstruction test split, mean per-spectrum reconstruction S/N rises from $7.6$ on the noisy input to $122$ on the blindspot output: a factor of about $16$, on a metric (Equation~\ref{eq:snr_def}) that compares each denoised spectrum against the withheld clean reference (Table~\ref{tab:main_baselines}). Figure~\ref{fig:representative_spectrum} shows one faint spectrum where the input drowns the Ca\,\textsc{ii} triplet and the output follows the line cores. The canonical reconstruction test split contains $10{,}000$ spectra with a single noise realization per spectrum and no clean reference at training time (Appendix~\ref{app:contract}).
...
\caption{Line-sensitive and high-noise-window reconstruction diagnostics on the test split. Equivalent-width entries use the relative error in Equation~\ref{eq:ew_relative_error}; polarity flips use Equation~\ref{eq:polarity_flip}. The \mbox{O$_2$} A-band rows are an atmospheric-window stress test, not a stellar-line diagnostic. Per-line bootstrap confidence intervals, polarity subgroups, and the Ca\,\textsc{ii} morphology panel are reported in Appendix~\ref{app:line_addenda}.}
...
Reconstruction signal-to-noise rises with estimator capacity, but the downstream $\log g$ readout separates methods that preserve label information from those that only raise pixel S/N: the principal-component references reach the highest reconstruction S/N among the non-neural methods yet leave the calibrator no more accurate than the noisy input, whereas the blindspot moves the calibrator most of the way to the clean-input scale.
...
% \begin{tabular}{lcccccc}
% \hline
% Method & Mean S/N & Median S/N [$5$,$95$] & Ca\,\textsc{ii} EW & Polarity & $\log g\ \sigma_{\rm rob}$ & $\log g\ R^2$ \\
% \hline
% Clean reference & --- & --- & --- & --- & $0.22$ & $0.94$ \\
% Noisy input & $7.6$ & $6.8$ [$3.1$,$14.6$] & $3.09$ & $0.282$ & $1.26$ & $0.01$ \\
