...
The clean-template PCA reference bounds what supervision alone buys a fixed-rank linear estimator: built from noiseless training spectra, it carries clean targets the blindspot never sees, yet reaches only $55.7$, within a few percent of the noisy-only PCA at $54.0$ and about a factor of two below the blindspot output. The reconstruction gain over the linear and classical-filter baselines is therefore not an artefact of access to clean training data. The line-preservation diagnostics in Section~\ref{sec:results_physical} further establish that this gain is genuine line recovery rather than smoothing toward the continuum, and the central-pixel exclusion behind the reconstruction is verified numerically in Appendix~\ref{app:architecture_integrity}. Why the fixed-basis estimators cannot close the remaining gap to the blindspot output --- with or without clean targets, and even when granted a rest-frame alignment step --- is taken up in Section~\ref{sec:discussion}.
